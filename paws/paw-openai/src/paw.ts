...
export const paw: PawDefinition = {
	...
	async think(context: AgentContext): Promise<AgentPlan> {
		...
		try {
			...
			// Get max context from config, env, or default (128K for OpenAI)
			const maxContextTokens = configMax || Number(process.env.BRAIN_MAX_CONTEXT) || 128000

			// Trim messages to fit within context limit
			const totalTokens = estimateMessageTokens(openaiMessages) + toolTokens
			if (totalTokens > maxContextTokens) {
				console.warn(
					`[paw-openai] context ${totalTokens} tokens exceeds limit ${maxContextTokens}, trimming oldest messages`,
				)
				openaiMessages = trimToFit(openaiMessages, maxContextTokens, toolTokens)
			}

			console.log(
				`[paw-openai] chat request — model: ${model}, messages: ${openaiMessages.length}, tools: ${openaiTools.length}, ~${estimateMessageTokens(openaiMessages) + toolTokens} tokens`,
			)
